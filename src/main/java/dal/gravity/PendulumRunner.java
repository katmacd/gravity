...
public class PendulumRunner {
	...
	public static void main(String[] args) {
		...
		GravityModel daGM = new GravityConstant(9.81);
		
		double delta = (args.length == 0) ? .1 : Double.parseDouble(args[0]);
		double sLen = 10, pMass = 10, theta0 = Math.PI / 30;
		RegularPendulum rp = new RegularPendulum(sLen, pMass, theta0, delta, daGM.getGravitationalField(), daGM);
		SimplePendulum sp = new SimplePendulum(sLen, pMass, theta0, daGM);
		RegularPendulum rpCoarse = new RegularPendulum(sLen, pMass, theta0, .1, daGM.getGravitationalField(), daGM);
